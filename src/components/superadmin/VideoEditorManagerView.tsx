...
interface EnrichedStaffProfile { 
    uid: string;
    name: string;
    email: string;
    mobile?: string;
...
const VideoEditorManagerView = () => {
    const [videoEditors, setVideoEditors] = useState<EnrichedStaffProfile[]>([]);
    ...
    // Placeholder data
    const placeholderEditors: EnrichedStaffProfile[] = [
        { uid: 'editor-1', name: 'Aarav Sharma', email: 'aarav@example.com', stats: { assigned: 5, pending: 2, completed: 3 } },
        { uid: 'editor-2', name: 'Priya Patel', email: 'priya@example.com', stats: { assigned: 8, pending: 1, completed: 7 } },
        { uid: 'editor-3', name: 'Rohan Mehta', email: 'rohan@example.com', stats: { assigned: 3, pending: 3, completed: 0 } },
    ];
    const placeholderCampaigns = [
        { id: 'camp-1', name: 'Summer Sale Campaign' },
        { id: 'camp-2', name: 'Diwali Special Promo' },
        { id: 'camp-3', name: 'New Product Launch' },
    ];
    const placeholderStaffCampaigns = [
        { id: 'staff-camp-1', name: 'Assigned Task A', status: 'In-Progress' },
        { id: 'staff-camp-2', name: 'Assigned Task B', status: 'Submitted' },
    ];
    ...
    const fetchStaffData = useCallback(async () => {
        ...
        setVideoEditors(placeholderEditors);
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <div className="mb-8">
                <h1 className="text-3xl font-bold text-slate-800 tracking-tighter">Video Editor Management</h1>
                <p className="text-slate-500 mt-1">Manage and monitor video editor performance and assignments</p>
            </div>
                        ...
                        >
                            <div className="flex items-center justify-between mb-4">
                               <div className="flex items-center">
                                     <div className="w-10 h-10 bg-red-100 rounded-full flex items-center justify-center mr-3 border-2 border-white">
                                        <span className="text-red-600 font-semibold text-sm">{editor.name?.charAt(0)?.toUpperCase() || 'V'}</span>
                                    </div>
                                    <div>
                                        <h3 className="font-semibold text-slate-900">{editor.name || 'Unnamed Editor'}</h3>
                                        <p className="text-sm text-slate-500">ID: {editor.uid.slice(-6)}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-4 mb-6 text-center bg-white/30 p-3 rounded-lg">
                                <div>
                                    <div className="text-2xl font-bold text-slate-800">{editor.stats.assigned}</div>
                                    <div className="text-xs text-slate-500 uppercase font-semibold">Assigned</div>
                                </div>
                                <div>
                                    <div className="text-2xl font-bold text-yellow-600">{editor.stats.pending}</div>
                                    <div className="text-xs text-slate-500 uppercase font-semibold">Pending</div>
                                </div>
                                <div>
                                    <div className="text-2xl font-bold text-green-600">{editor.stats.completed}</div>
                                    <div className="text-xs text-slate-500 uppercase font-semibold">Completed</div>
                                </div>
                            </div>
                             <div className="flex space-x-3">
                                <button onClick={() => {
                                  setSelectedStaff(editor);
                        ...
                        <motion.div className="bg-white/60 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-md border border-slate-300/70" initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}>
                            ...
                            <form className="p-6" onSubmit={async (e) => {
                                ...
                                <div className="mt-4">
                                    ...
                                    {staffCampaigns.length === 0 ? (
                                        <p className="text-sm text-slate-500">No campaigns assigned.</p>
                                    ) : (
                                        <ul className="space-y-2 max-h-32 overflow-y-auto p-2 bg-white/20 rounded-lg">
                                            {staffCampaigns.map(campaign => (
                                                <li key={campaign.id} className="p-2 bg-white/30 rounded text-sm text-slate-800">{campaign.name} - {campaign.status}</li>
                                            ))}
                                        </ul>
                                    )}
                                </div>

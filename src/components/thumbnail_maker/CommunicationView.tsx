...
interface Message {
    ...
    message: string;
    timestamp: string;
    type: 'brand' | 'writer' | 'editor' | 'admin' | 'thumbnail_maker';
...
const CommunicationView = () => {
    ...
    useEffect(() => {
        // Placeholder data
        setUserProfile({ name: 'Demo Thumbnail Maker', email: 'thumb@example.com' });
        const placeholderTasks: Task[] = [
            { id: 'THUMB001', campaign: 'Summer Sale', brand: 'FashionCo', status: 'In-Progress', participants: ['Editor', 'Writer', 'Thumbnail Maker'] },
            { id: 'THUMB002', campaign: 'Product Launch', brand: 'TechGadgets', status: 'Submitted', participants: ['Editor', 'Admin', 'Thumbnail Maker'] }
        ];
    ...
    const startNewChat = () => {
        const newId = `THUMB${(parseInt(tasks[0]?.id.slice(5) || '0') + 1).toString().padStart(3, '0')}`;
        const newTask: Task = {
            id: newId,
            campaign: 'Untitled Campaign',
            brand: 'New Client',
            status: 'New',
            participants: ['Video Editor', 'Thumbnail Maker', 'Brand Manager'],
        };
    ...
    const handleSendMessage = () => {
        ...
        const newMessage: Message = {
            id: Date.now(),
            sender: 'Thumbnail Maker',
            message: message,
            timestamp: new Date().toLocaleString(),
            type: 'thumbnail_maker',
        };
    ...
    const getMessageStyle = (type: string) => {
        switch (type) {
            ...
            case 'thumbnail_maker': return 'bg-orange-50 border-orange-200';
                ...
                {selectedTask ? (
                    <>
                                    ...
                                    {selectedTask.participants.map(participant => (
                                        <div key={participant} className="flex items-center space-x-1 text-sm text-slate-600">
                                            <span>{getSenderIcon(
                                                ...
                                                participant === 'Video Editor' ? 'editor' : 
                                                participant === 'Thumbnail Maker' ? 'thumbnail_maker' : 'admin'
                                            )}</span>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
                            {messages.map(msg => (
                                <div key={msg.id} className={`flex ${msg.type === 'thumbnail_maker' ? 'justify-end' : 'justify-start'}`}>
                        ...
                        <div className="bg-white border-t border-slate-200 p-4">
                            <div className="space-y-3">
                                <div className="flex space-x-2">
                                    ...
                                    >
                                        <Paperclip/> <span>Share Media</span>
                                    </button>
                                    <button
                                    ...
                                    >
                                        <AlertTriangle/> <span>Report Issue</span>
                                    </button>
                                </div>
                                <div className="flex space-x-2">
                                    <input

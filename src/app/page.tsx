...
import { LoaderCircle, ArrowRight, Bot, BarChart, Clock, Search, Mail, Layers, Trophy, Instagram, Facebook, Youtube, Twitter } from 'lucide-react';
...
function LandingPage() {
  return (
     <div className="flex flex-col min-h-screen text-foreground bg-slate-200 bg-gradient-to-br from-white/30 via-transparent to-transparent">
        <PublicHeader />
 ...
 * for logged-in users, or shows the landing page for guests.
 */
export default function HubPage() {
  ...
  useEffect(() => {
    // Wait until we have all auth and profile info
    if (isUserLoading || (user && isProfileLoading)) {
      return;
    }
    
    // If a logged-in user exists, redirect them based on their role
    if (user && userProfile && userProfile.role) {
      switch (userProfile.role) {
        ...
        // User is logged in but profile doesn't exist yet, redirect to a default panel
        console.warn("User is logged in but no profile found. Redirecting to default panel.");
        router.replace('/uploader_panel');
    }
    // If no user is logged in, the component will just render the LandingPage.

  }, [user, isUserLoading, userProfile, isProfileLoading, router]);

  // If we are still figuring out the user's auth state, show a loading screen.
  // We check for `user` as well to prevent a flash of the landing page for logged-in users.
  if (isUserLoading || (user && isProfileLoading)) {
    return (
